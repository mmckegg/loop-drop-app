...
function lambda (...inputs) {
  ...
  var numberResult = numbers.reduce(sum, 0)
  if (params.length > 0) {
    var hasNumbers = numbers.length > 0
    var previousHasNumbers = this.numberResult
    if (this.result && deepEqual(params, this.inputs) && hasNumbers === previousHasNumbers) {
      ...
      this.result = sumParams(params, this.numberResult)
      return this.result
    }
  } else {
    this.result = null
...
function sumParams (params, extraParam) {
  var audioContext = params[0].context
  var result = audioContext.createGain()

  for (var i = 0; i < params.length; i++) {
    params[i].connect(result)
  }

  if (extraParam) {
    extraParam.connect(result)
  }
