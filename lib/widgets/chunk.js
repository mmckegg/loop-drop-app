...
module.exports = function (node, opts) {
  var instance = node.node || node
  var actions = node.context.actions

  var selected = computed([instance.id, node.context.setup.selectedChunkId], eq)
  var minimised = instance.minimised
  var volume = instance.volume
  var color = instance.color

  return h('div ExternalNode', {
    classList: [
      when(minimised, '-minimised'),
      when(selected, '-selected')
    ],
    'ev-click': send(selectChunk, instance),
    'style': {
      border: computed([color, selected], function (color, selected) {
    ...
    h('header', {
      style: {
        'background-color': computed([color, selected], function (color, selected) {
