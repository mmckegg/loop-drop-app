...
function lambda (inputs) {
  ...
  if (nodes.length > 0) {
    if (!this.numberResult) {
      ...
      this.getValueAtTime = ValueAtTimeGetter(this.nodes, this.numberResult, this.reduceValues)
      watch(this.result, (node) => {
        // HACK: add getValueAtTime to AudioNodes
        if (isAudioNode(node) && !node.getValueAtTime) {
          node.getValueAtTime = this.getValueAtTime
        }
      })
    } else {
