...
function getParamLinkOptions (context, currentValue, format) {
  var options = computed([context.paramLookup, currentValue], function (params, currentValue) {
    var result = []
    var currentId = currentValue && currentValue.node === 'linkParam' ? currentValue.param : null
    var included = false
    var keys = context.paramLookup.keys()
    for (var i = 0; i < keys.length; i++) {
      var p = context.paramLookup.get(keys[i])
      var id = keys[i]
      if (!hasFeedback(context, p)) {
        ...
        result.push(
          h('option', {
            rawValue: { key: id, format: format },
            action: Value(setParamLink),
            selected: currentValue && currentValue.node === 'linkParam' && currentValue.param === id
    ...
    if (!included && currentId) {
      result.unshift(
        h('option', {
          rawValue: { key: currentId, format: format },
          action: Value(setParamLink),
          selected: true
        }, '[' + currentId + ']')
      )
...
function spawnParam (param, options) {
  ...
  setParamLink(param, { key: key, format: options.format })
}
...
function spawnModulatorChunk (param, options) {
  ...
  spawnNode(setup.chunks, 'modulatorChunk', index, function (err, chunk) {
    if (options.node) {
      var newId = setup.chunks.resolveAvailable(resolve(options.node.id) + '-' + (options.title || 'modulator'))
