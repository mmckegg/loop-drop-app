...
function ScaleChooser (param) {
  var value = computed([param], getValue)

  var buttons = []
  for (var i = 0; i < 12; i++) {
    var selected = getPresent(value, i)
...
function getPresent (collection, item) {
  return computed([collection], value => value.includes(item))
}
