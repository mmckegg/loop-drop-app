...
var recordNodeToDisk = require('lib/record-node-to-disk')
var recordExternalAudioToDisk = require('lib/record-external-audio-to-disk')
...
function SessionRecorder (context, active) {
  ...
  function refreshRecording () {
    ...
    if (isRecording()) {
      ...
      var externalPath = join(recordingsPath, strftime('%F %H%M%S') + '-external.json')

      clips.push(path)
      obs.recordPosition.set(0)
      releases.push(
        recordNodeToDisk(path, obs.input, onTick),
        recordExternalAudioToDisk({
          deviceId: -1,
        ...
        writeTimeCodeToDisk(context, externalPath + '.time')
      )
    }
  }
