...
function TemplateSlot (context, shape, scale) {
  ...
  var items = computed([deepMatch(shape), deepMatch(scale), obs.nodeName], function (shape, scale, nodeName) {
    ...
    for (var i = 0; i < length; i++) {
      ...
      if (!deepEqual(item, this.result[i])) {
        this.result[i] = item
      }
    }
  ...
  obs.slots = MutantMap(items, function (item) {
    var ctor = resolveNode(context.nodes, item[0])

    if (ctor) {
      var result = ctor(context)
      var value = computed([throttledValue, item[1]], obtainWithParams)
