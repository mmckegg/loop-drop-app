...
function Param (parentContext, defaultValue) {
  ...
  obs.getValueAtTime = function (time) {
    var value = obs.currentValue()
    if (typeof value === 'number') {
      return value
    } else if (value.getValueAtTime) {
      return value.getValueAtTime(time)
    } else {
      return 0
    }
  }
...
Param.triggerOn = function (obj, at) {
  for (var k in obj) {
    if (obj[k] && k !== 'templateSlot' && obj[k].triggerOn) {
      obj[k].triggerOn(at)
    }
  }
}
...
Param.triggerOff = function (obj, stopAt) {
  for (var k in obj) {
    if (obj[k] && k !== 'templateSlot' && obj[k].triggerOff) {
