  ...
  Behave = Behave || function (userOpts) {
        ...
        charFuncs = {
            openedChar: function (_char, e) {
                ...
                var start = sel ? sel.start : pos
                var end = sel ? sel.end : pos

                var left = val.substring(0, start)
                var content = val.slice(start, end)
                var right = val.substring(end)
                var edited = left + _char.open + content + _char.close + right

                defaults.textarea.value = edited;
                if (content){
                    utils.cursor.set(start, end + 2);
                } else {
                    utils.cursor.set(start + 1);
                }
    ...
    if (typeof module !== 'undefined' && module.exports) {
        module.exports = function(textarea, options){
          options = options || {}
          options.textarea = textarea
          return new Behave(options)
        }
    }
